...
// whose card is open. The shadow variant is a soft dark disc, premultiplied.
...
void main() {
  if (uFootprint > 0.0) {
    ...
    float a = 0.3 * smoothstep(1.0, 0.3, r);
  ...
  vec3 col = vColor * (0.48 + 0.62 * diff) + vColor * 0.08 * max(n.y, 0.0);
