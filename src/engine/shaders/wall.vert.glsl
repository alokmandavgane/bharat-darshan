...
uniform vec4 uLocalRect;     // country-uv rect the bound heightmap covers; (0,0,1,1) is the country tier
...
void main() {
  ...
  float top = lift(texture(uHeight, luv(st)).r) + uLift;
  float y = position.y > 0.5 ? top : uLift - uDepth;
  vT = position.y;
  vLit = 0.72 + 0.28 * dot(normalize(side), normalize(uLightDir));   // the key light, which turns with the camera
